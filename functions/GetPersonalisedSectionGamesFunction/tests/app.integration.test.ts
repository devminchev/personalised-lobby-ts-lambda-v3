...
import { EXPECTED_ML_SUGGESTED_RESPONSE } from './mocks/expectedAPIResponse';
import {
...
const SECTION_NON_PERSONALISED_ID = '7H8um3qZjY7dpedPoL5tw';
...
describe('Integration Test for Lambda Handler', () => {
    ...
    describe('Client error handling for 4xx status codes', () => {
        ...
        it('Should return (400) to the client if sectionid is missing', async () => {
            const event: APIGatewayProxyEvent = mockApiEvent(SITE_NAME, PLATFORM, VIEW_SLUG, undefined, MEMBER_ID);
            const result: APIGatewayProxyResult = await lambdaHandler(event);
            const body = parseCompressedBody<{ code: string; message: string }>(result);
        ...
        it('should return (204) for invalid SECTION_ID', async () => {
            ...
            const event: APIGatewayProxyEvent = mockApiEvent(SITE_NAME, PLATFORM, VIEW_SLUG, 'invalid', MEMBER_ID);
            const result: APIGatewayProxyResult = await lambdaHandler(event);
        ...
        it('Should return (400) for a non-personalised section type', async () => {
            nock('http://localhost:9200')
                .post(`/${ALL_SECTIONS_SHARED_READ_ALIAS}/_search?request_cache=true`)
