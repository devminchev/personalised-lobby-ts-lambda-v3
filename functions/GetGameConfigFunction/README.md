# Get-gameconfig Lambda Function

> A GET endpoint that provides game configuration data.

The lambda retrieves the categories for a specific venture for the endpoint `/game-config/{sitegameid}`.
...
```sh
    sam local invoke "GetGameConfigFunction" -e functions/GetGameConfigFunction/events/event.json --env-vars env.json
```
