 ...
 *   - The built bundle in `dist/app.cjs` talks to a real DynamoDB instance
 ...
 *   3. Sets env vars so the handler (imported from ../dist/app.cjs) talks
 ...
 *
 * Usage:
 *   yarn build              # must be current; the script uses dist/app.cjs
 *   yarn test:e2e
 *
 * Flags:
...
const CB_DDB_TABLE = 'CircuitControl-e2e';
...
process.env.CB_DDB_TABLE = CB_DDB_TABLE;
process.env.DDB_ENDPOINT = ENDPOINT;
// Provide all six BREAKER_* env vars so loadBreakerSettings short-circuits
// to env-derived values without contacting SSM. Values match the previously-
// hardcoded BreakerSettings defaults so the existing scenario expectations
// (holdUntil offsets, TTL deltas, etc.) remain valid. BREAKER_STALE_EVENT_MS=0
// disables the freshness guard so test fixtures with any timestamp are accepted.
// SIGNAL_MAX_AGE_MS is independent (it would otherwise default to
// 2×staleEventMs = 0, which would disable zombie pruning — we want it on
// for the zombie test).
...
process.env.BREAKER_EVENT_HISTORY_TTL_SEC = String(14 * 24 * 60 * 60);
process.env.BREAKER_STATE_HISTORY_TTL_SEC = String(30 * 24 * 60 * 60);
...
const distPath = path.join(__dirname, '..', 'dist', 'app.cjs');
let handler;
try {
    ({ lambdaHandler: handler } = require(distPath));
} catch (err) {
...
async function wipeTable() {
    ...
    while (true) {
        const r = await ddb.send(new ScanCommand({ TableName: CB_DDB_TABLE, ExclusiveStartKey: start }));
        if (r.Items && r.Items.length > 0) {
            for (let i = 0; i < r.Items.length; i += 25) {
                await ddb.send(
                    new BatchWriteCommand({
                        RequestItems: {
                            [CB_DDB_TABLE]: r.Items.slice(i, i + 25).map((item) => ({
...
async function seedBreakerState(partial) {
    ...
    await ddb.send(new PutCommand({ TableName: CB_DDB_TABLE, Item: item }));
    return item;
}
...
async function testDefensiveGlobalCollapse() {
    ...
    const scan = await ddb.send(new ScanCommand({ TableName: CB_DDB_TABLE }));
...
async function main() {
    ...
    if (!SKIP_SETUP) {
        ...
        console.log(`${YELLOW}Creating table ${CB_DDB_TABLE}...${RESET}`);
        createTable();
    } else {
